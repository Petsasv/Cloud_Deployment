...
| Path | Purpose |
|---|---|
| `Terraform/` | All `.tf` files: EC2 instances, security groups, IAM, key pair, S3 state backend, DynamoDB lock table, variables |
| `Website/` | Node.js / Express application — `server.js`, static HTML/CSS/JS frontend, MongoDB-backed login/register endpoints |
| `.github/workflows/` | GitHub Actions pipeline that runs Terraform on infrastructure changes |
| `scripts/` | Bootstrap script for the Linux host (installs Node, MongoDB, pm2, clones the repo, starts the app) and CodeDeploy lifecycle hooks |
| `appspec.yml` | AWS CodeDeploy specification for application deployments |
...
1. Checkout
2. Set up Terraform (pinned to `1.11.4`)
